...
public class MapWidget extends Canvas {
	...
	public class TileCache {
		...
		public void put(TileServer tileServer, int x, int y, int z,
				AsyncImage image) {
	...
	public final class AsyncImage implements Runnable {
		...
		public void run() {
			...
			if (stamp != zoomStamp.longValue()) {
				...
				try {
					...
					log.log(Level.INFO,
							"swt exception during redraw display-race, ignoring");
				}
		...
		public void dispose(Display display) {
			checkThread(display);
			if (image != null) {
				// System.err.println("disposing: " + getTileString(tileServer,
				// x, y, z));
				image.dispose();
			}
		}
		...
		private void checkThread(Display display) {
			...
			if (display.getThread() != Thread.currentThread()) {
				throw new IllegalStateException(
						"wrong thread to pick up the image");
			}
		}
	}

	private class MapMouseListener implements MouseListener,
			MouseWheelListener, MouseMoveListener, MouseTrackListener {
	...
	public static String getTileString(TileServer tileServer, int xtile,
			int ytile, int zoom) {
	...
	public PointD getLongitudeLatitude(Point position) {
		return new PointD(position2lon(position.x, getZoom()), position2lat(
				position.y, getZoom()));
	}
	...
	public static String getTileNumber(TileServer tileServer, double lat,
			double lon, int zoom) {
